...
export default function HospitalsPage() {
  ...
  const fetchHospitals = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000"}/api/hospitals?${params}`,
  ...
  const handleDelete = async (id: string) => {
    ...
    const token = getToken();
    await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000"}/api/hospitals/${id}`, {
      method: "DELETE",
  ...
  const handleToggleStatus = async (id: string, isActive: boolean) => {
    const token = getToken();
    await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000"}/api/hospitals/${id}/status`, {
      method: "PATCH",
